...
class ScreenLogin : AppCompatActivity() {
    ...
    /*private fun loginUser(emailOrUsername: String, password: String) {
        val usuario = Usuario(emailOrUsername, "", password, "")

        RetrofitClient.instance.loginUsuario(usuario).enqueue(object : Callback<String> {
            override fun onResponse(call: Call<String>, response: Response<String>) {
                if (response.isSuccessful) {
                    val intent = Intent(this@ScreenLogin, ScreenPrincipal::class.java)
                    startActivity(intent)
                    finish()
                } else {
