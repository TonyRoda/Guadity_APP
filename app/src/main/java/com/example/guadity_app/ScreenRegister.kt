...
class ScreenRegister : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        /*val button: MaterialButton = findViewById(R.id.btn_registrarse)
        button.setOnClickListener {
            val nombre = nombreUsuario.text.toString().trim()
            val email = correoElectronico.text.toString().trim()
            val password = contrasena.text.toString().trim()
            ...
            // Opcional: Validar edad mínima (ej. 13 años)
            if (!isFechaNacimientoValida(fecha)) {
                MotionToast.createColorToast(
                    this,
                    "ADVERTENCIA",
            ...
            // Llama a la función para registrar
            registerUser(nombre, email, password, fecha)
        }*/
        ...
        /*val call = RetrofitClient.instance.listarEquipos(id)
    ...
    /*private fun registerUser(nombre: String, email: String, password: String, fecha: String) {
        val usuario = Usuario(nombre, email, password, fecha)

        RetrofitClient.instance.registrarUsuario(usuario).enqueue(object : Callback<String> {
